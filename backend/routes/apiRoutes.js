...
// to get all the jobs [pagination] [for admin personal and for everyone]
router.get("/jobs", jwtAuth, (req, res) => {
  ...
  // to list down jobs posted by a particular admin
  if (user.type === "admin" && req.query.myjobs) {
    findParams = {
      ...findParams,
      userId: user._id,
    };
...
// admin gets applications for a particular job [pagination] [todo: test: done]
...
// admin/healthworker gets all his applications [pagination]
router.get("/applications", jwtAuth, (req, res) => {
  ...
  Application.aggregate([
    ...
    {
      $match: {
        [user.type === "admin" ? "recruiterId" : "userId"]: user._id,
      },
    },
    {
      $sort: {
...
// get a list of final applicants for current job : admin
// get a list of final applicants for all his jobs : admin
                        ...
                        JobApplicant.findOneAndUpdate(
                          ...
                          .then((healthworker) => {
                            if (healthworker === null) {
                              res.status(400).json({
                                message:
                                  "Error while updating healthworker's average rating",
                              });
                              return;
                            }
                            res.json({
...
router.get("/rating", jwtAuth, (req, res) => {
  ...
  Rating.findOne({
    ...
    category: user.type === "admin" ? "healthworker" : "job",
  }).then((rating) => {
    if (rating === null) {
      res.json({
        rating: -1,
